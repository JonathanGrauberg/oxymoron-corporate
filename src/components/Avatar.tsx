...
export default function Avatar({ name, photo, size = 88, grayscale = false, className = "" }: AvatarProps) {
  return (
    <div
    ...
    >
      {photo ? (
        <img
          src={photo}
          alt={name}
          className={`w-full h-full object-cover object-top ${grayscale ? "grayscale" : ""}`}
        />
      ) : (
